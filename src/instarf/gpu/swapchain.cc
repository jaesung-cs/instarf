...
Swapchain::Swapchain(const Device& device, VkSurfaceKHR surface) : instance_(device.instance()), device_(device), surface_(surface), queue_(device.queue()) {
  imageCount_ = 3;
  ...
  swapchainInfo_.imageUsage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
  ...
  for (uint32_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    vkCreateSemaphore(device, &semaphoreInfo, nullptr, &imageAcquiredSemaphores_[i]);
  }

  frameFinishedFences_ = std::vector<VkFence>(MAX_FRAMES_IN_FLIGHT, VK_NULL_HANDLE);
}
...
Swapchain::~Swapchain() {
  ...
  if (!renderingFences.empty()) {
    vkWaitForFences(device_, static_cast<uint32_t>(renderingFences.size()), renderingFences.data(), VK_TRUE, UINT64_MAX);
  }

  for (auto semaphore : imageAcquiredSemaphores_) vkDestroySemaphore(device_, semaphore, nullptr);
  for (auto semaphore : renderFinishedSemaphores_) vkDestroySemaphore(device_, semaphore, nullptr);
  for (auto fence : renderFinishedFences_) vkDestroyFence(device_, fence, nullptr);
...
bool Swapchain::resize(uint32_t width, uint32_t height) {
  if (swapchainInfo_.imageExtent.width != width || swapchainInfo_.imageExtent.height != height) {
    std::vector<VkFence> renderingFences;
    for (auto fence : frameFinishedFences_) {
      if (fence) renderingFences.push_back(fence);
    }
    ...
    if (!renderingFences.empty()) {
      vkWaitForFences(device_, static_cast<uint32_t>(renderingFences.size()), renderingFences.data(), VK_TRUE, UINT64_MAX);
      vkResetFences(device_, static_cast<uint32_t>(renderingFences.size()), renderingFences.data());
    }
...
bool Swapchain::begin() {
  ...
  auto result = vkAcquireNextImageKHR(device_, swapchain_, UINT64_MAX, imageAcquiredSemaphores_[frameIndex], nullptr, &imageIndex_);
...
void Swapchain::end() {
  ...
  if (frameFinishedFences_[frameIndex]) {
    vkWaitForFences(device_, 1, &frameFinishedFences_[frameIndex], VK_TRUE, UINT64_MAX);
  ...
  vkQueueSubmit(queue_, 1, &submitInfo, renderFinishedFences_[imageIndex_]);
...
void Swapchain::destroyImageViews() {
  for (auto imageView : imageViews_) vkDestroyImageView(device_, imageView, nullptr);
  images_.clear();
  imageViews_.clear();
}
}  // namespace gpu
